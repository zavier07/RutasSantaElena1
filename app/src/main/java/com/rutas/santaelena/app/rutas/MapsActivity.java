...
public class MapsActivity extends AbstractAsyncActivity implements OnMapReadyCallback, NavigationView.OnNavigationItemSelectedListener {
    ...
    public void onMapReady(final GoogleMap googleMap) {
        ...
        placeAutoCompleteOrigenDestino.setHint("Busca tus sitios aquí");
    ...
    public void onClick(View v) {
        if (markerPointsOrigenDestino.size() <= 1)
            Toast.makeText(getApplicationContext(), "Primero Escoga su Punto Origen y Destino en el Mapa", Toast.LENGTH_LONG).show();
        else
            new SearchPlacesParaderosNear().onClick(v, mMap, markerPointsOrigenDestino, this, markerOrigen, markerFindelViajeEnbus,markerFindelViajeEnbus,markerFindelViajeEnbus);
    }
    ...
    private void encuentraRutasAlPuntoOrigenOrigen(LatLng pointOrigenMapa, int radio){
            ...
            if (origenRutaEncontrado)
                markerOrigen = colocarMarker.colocarMarcadorPuntoOrigenEnMapa(pointOrigenMapa, " Punto de Partida: ", mMap, this);
            else
