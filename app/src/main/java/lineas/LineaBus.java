...
public class LineaBus extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private ArrayList<LatLng> listaPuntosXY = new ArrayList<>();
        ...
        AsyncTask<Object, Void, Ruta> httpLineas = new HttpGetLinea(new HttpGetLinea.AsyncResponse2() {
            ...
            public void processFinish(Ruta rutaModel) {
                if (rutaModel != null) {
                    ...
                    paraderosWpt(listParadas);

                }
                else
                    Toast.makeText(getApplicationContext(),"RUTA NO DISPONIBLE POR EL MOMENTO",Toast.LENGTH_SHORT).show();
            }
    ...
    public void polilineaRestful(List<Point> listasPuntos) {
        ...
        for (int i = 0; i < listasPuntos.size(); i++) {
            ...
            listaPuntosXY.add(punto);
        }
        mMap.addPolyline(new PolylineOptions().addAll(listaPuntosXY).width(5).color(Color.BLUE));
        Toast.makeText(getApplicationContext(),"SE MUESTRA RECORRIDO DE LA LINEA " + lineaBus,Toast.LENGTH_SHORT).show();
    }


    private void paraderosWpt(List<String> listParaderosWpt){

        for (int i = 0; i < listParaderosWpt.size(); i++) {

            AsyncTask<Object, Void, Parada> httpGetParadasLinea = new HttpGetParadasLinea(new HttpGetParadasLinea.AsyncResponse2() {
                @Override
                public void processFinish(Parada parada) {
                    dibujaParaderos(parada);
                }
            }).execute(listParaderosWpt.get(i),this);

        }
    }

    private void dibujaParaderos(Parada parada){

        double latitude =  parada.getCoordenada().getY();
        double longitude = parada.getCoordenada().getX();

        LatLng punto = new LatLng(latitude, longitude);

        new Marcador().colocarParaderosRutaBus(punto,"Paradero linea : " + lineaBus,mMap,this);
